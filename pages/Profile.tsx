...
import { User as UserIcon, Mail, Phone, Lock, Save, Building2, BookOpen, Fingerprint, CreditCard, Briefcase, MapPin } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { ROLE_LABELS } from '../constants';

const PREDEFINED_PROGRAM = "Ijazah Sarjana Muda Teknousahawanan dengan Kepujian";

// Moved InputField outside so it doesn't get re-created on every render
...
export const Profile: React.FC<ProfileProps> = ({ user, onUpdateUser }) => {
  ...
  // Initialize Program Selection on load
  useEffect(() => {
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {formData.role === UserRole.STUDENT && (
                ...
                <div className="space-y-1">
                    ...
                    {isEditing ? (
                        ...
                        <div className="relative">
                            ...
                            <input disabled className="w-full pl-10 pr-4 py-2 border border-transparent bg-slate-50 rounded-lg text-slate-500" value={formData.program} />
                        </div>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    onChange={(v: string) => setFormData({...formData, matric_no: v})} 
                    isEditing={isEditing}
                    />
                    <InputField 
                    ...
                    onChange={(v: string) => setFormData({...formData, ic_no: v})} 
                    isEditing={isEditing}
                    />
                </div>
                ...
                <div className="space-y-1">
                     ...
                     <div className="relative">
                        ...
                        <textarea
                            disabled={!isEditing}
                            className={`w-full pl-10 pr-4 py-2 border rounded-lg transition-colors ${
                                !isEditing ? 'bg-slate-100 text-slate-500 border-transparent' : 
            ...
            {(formData.role === UserRole.LECTURER || formData.role === UserRole.TRAINER) && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <InputField 
                    label="No. Staf / ID Pekerja" 
                    icon={CreditCard} 
                    value={formData.staff_id} 
                    onChange={(v: string) => setFormData({...formData, staff_id: v})} 
                    isEditing={isEditing}
                    />
                </div>
                </section>
            ...
            {(formData.role === UserRole.TRAINER || formData.role === UserRole.SUPERVISOR) && (
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     ...
                     onChange={(v: string) => setFormData({...formData, company_position: v})} 
                     isEditing={isEditing}
                     />
                 </div>
                 </section>
            ...
            {isEditing && (
              <div className="flex justify-end pt-4">
                <button 
                  type="submit"
                  className="flex items-center gap-2 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-bold transition-transform active:scale-95 shadow-lg shadow-green-200"
                >
                  <Save size={20} />
                  Simpan Perubahan
                </button>
