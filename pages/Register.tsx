...
import { MapPin, Sparkles, Building2, GraduationCap, Clock } from 'lucide-react';
...
export const Register: React.FC<RegisterProps> = ({ onRegisterSuccess, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (needsApproval) {
          toast.success('Pendaftaran berjaya! Akaun anda sedang menunggu kelulusan Penyelaras.');
      } else {
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-2xl my-8 animate-fadeIn">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {formData.role === UserRole.STUDENT && (
            <div className="p-4 bg-slate-50 rounded-lg space-y-3 border border-slate-200">
               ...
               <div>
                  ...
                  <textarea required className="w-full p-2 border rounded bg-white" rows={3} placeholder="Alamat penuh..." onChange={e => setFormData({...formData, address: e.target.value})} />
               ...
               {suggestedCompanies.length > 0 && (
                 <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 animate-fadeIn">
                    <div className="flex items-center gap-2 mb-2 text-blue-700 font-bold text-sm">
                      <Sparkles size={16} /> Cadangan Syarikat Berhampiran (AI)
                    </div>
          ...
          {(formData.role === UserRole.TRAINER || formData.role === UserRole.SUPERVISOR) && (
               <div className="p-4 bg-slate-50 rounded-lg space-y-3 border border-slate-200">
                  <h4 className="text-sm font-bold text-slate-600 uppercase tracking-wide flex items-center gap-2">
                      <Building2 size={16} /> Maklumat Industri
                  </h4>
                  <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                          <label className="block text-sm font-medium text-slate-700 mb-1">Tahap Akademik Tertinggi</label>
                      ...
                      <div>
                          <label className="block text-sm font-medium text-slate-700 mb-1">Pengalaman Bekerja (Tahun)</label>
                      ...
                      <label className="flex items-center gap-3 p-3 bg-blue-50 border border-blue-100 rounded cursor-pointer hover:bg-blue-100 transition-colors">
                          ...
                          <div>
                              <div className="text-sm font-bold text-slate-800">
                                  {formData.role === UserRole.SUPERVISOR ? "Bertindak juga sebagai Jurulatih Industri?" : "Bertindak juga sebagai Penyelia Industri?"}
                              </div>
                              <div className="text-xs text-slate-500">Tandakan jika anda memegang kedua-dua jawatan.</div>
                          </div>
                      </label>
                  </div>
               </div>
          ...
          <div className="flex gap-3 pt-4">
            <button type="button" onClick={onBack} className="flex-1 px-4 py-3 border border-slate-300 rounded-lg text-slate-700 font-medium hover:bg-slate-50 bg-white">Batal</button>
            <button type="submit" className="flex-1 px-4 py-3 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700">Daftar</button>
          </div>
        </form>
      </div>
    </div>
